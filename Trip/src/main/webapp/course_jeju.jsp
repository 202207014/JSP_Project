...
<body>
	...
	try {
		...
		// 쿼리: places 테이블에서 'jeju_'로 시작하는 장소(이름, 타입)만 조회
...
var courses = {
  "1day": [
    "성산 일출봉", "에코랜드 테마파크", "제주김녕미로공원", "한담해안 산책로", "돌담 흑돼지"
  ],
  "2day": [
    "성산 일출봉", "에코랜드 테마파크", "제주김녕미로공원", "한담해안 산책로", "대포해안 주상절리대",
    "제주 신라 호텔", // 숙박 포함
    "한라산 국립공원", "돌담 흑돼지"
  ],
  "3day": [
    "성산 일출봉", "에코랜드 테마파크", "제주김녕미로공원", "한담해안 산책로", "대포해안 주상절리대",
    "제주 신라 호텔", 
    "한라산 국립공원", "신창 풍차해안도로", "델문도"
  ]
};

// 카카오 지도 객체 생성 및 초기화 (제주도 중심 좌표 및 레벨 설정)
...
function loadCourse(courseName){
	...
	var course=courses[courseName]; // 선택된 코스 배열
	if(!course) return;
	
	course.forEach(function(name,index){
		// 1. DB에서 가져온 리스트(allPlaces)에서 해당 장소의 타입 정보를 찾음
